...
// Modified date : 2018-02-06 20:57
// Author : DARREN
// Describe : not set
...
_MakeEmpty(void* obj){
    if (NULL == obj){
        ...
        while(self->func->isEmpty(self) ){
            //unsigned int c = self->func->getCount(self);
            //INFO_LOG("count:%d",c);
            self->func->Pop(self);
        }
    }
...
Insert(void** node, void* obj){
    if (NULL == obj || NULL == node){
        ...
        nd->data = node;
        self->super->func->Insert(&nd->next,(void*)self->super);
    }
}

static
void
Append(void** node, void* obj){
    if (NULL == obj || NULL == node){
        CONDITION_VALUE_IS_NULL;
    }else{
        CLASS_TYPE* self = (CLASS_TYPE*)obj;
...
CLASS_NAME(int cnt){
    ...
    self->super = SUPER_CLASS_NAME(cnt);
